...
auto to_string(T&& val) -> std::string {
  ...
  return detail::to_stream(os, val).str();
}

TEST(AddressToString, ToStringInteger) {  // NOLINT
  ASSERT_EQ(to_string(uint8_t{0}), "0");
  ASSERT_EQ(to_string(uint8_t{0xFF}), "255");
  ASSERT_EQ(to_string(int8_t{-1}), "255");

  ASSERT_EQ(to_string(uint16_t{0x0000}), "0.0");
  ASSERT_EQ(to_string(uint16_t{0xFF00}), "255.0");
  ASSERT_EQ(to_string(int16_t{0x7F01}), "127.1");

  ASSERT_EQ(to_string(uint32_t{0x0}), "0.0.0.0");
  ASSERT_EQ(to_string(uint32_t{0x7F0000FF}), "127.0.0.255");
  ASSERT_EQ(to_string(int32_t{0x7F0000FF}), "127.0.0.255");

  ASSERT_EQ(to_string(uint64_t{0x7F0000FF}), "0.0.0.0.127.0.0.255");
  ASSERT_EQ(to_string(uint64_t{0xABCDEF0102030405}), "171.205.239.1.2.3.4.5");
}
...
TEST(AddressToString, ToStringString) {  // NOLINT
  ...
  ASSERT_EQ(to_string(""sv), ""s);
  ASSERT_EQ(to_string(""s), ""s);

  ASSERT_EQ(to_string("42"), "42"s);
  ASSERT_EQ(to_string("42"sv), "42"s);
  ASSERT_EQ(to_string("42"s), "42"s);

  ASSERT_EQ(to_string("Hello, World!"), "Hello, World!"s);

  {
    std::string hello_world = "Hello, World!";
    ASSERT_EQ(to_string(std::move(hello_world)), "Hello, World!"s);
  }
}

TEST(AddressToString, ToStringVector) {  // NOLINT
  ASSERT_EQ(to_string(std::vector<int>{}), "");
  ASSERT_EQ(to_string(std::vector<int>{42}), "42");
  ASSERT_EQ(to_string(std::vector<int>{42, 24}), "42.24");
  ASSERT_EQ(to_string(std::vector<int>{42, 24, 77}), "42.24.77");
}

TEST(AddressToString, ToStringList) {  // NOLINT
  ASSERT_EQ(to_string(std::list<int>{}), "");
  ASSERT_EQ(to_string(std::list<int>{42}), "42");
  ASSERT_EQ(to_string(std::list<int>{42, 24}), "42.24");
  ASSERT_EQ(to_string(std::list<int>{42, 24, 77}), "42.24.77");
}

TEST(AddressToString, ToStringDeque) {  // NOLINT
  ASSERT_EQ(to_string(std::deque<int>{}), "");
  ASSERT_EQ(to_string(std::deque<int>{42}), "42");
  ASSERT_EQ(to_string(std::deque<int>{42, 24}), "42.24");
  ASSERT_EQ(to_string(std::deque<int>{42, 24, 77}), "42.24.77");
}
...
TEST(IsContainerTest, HasEnd) {  // NOLINT
  ...
  static_assert(detail::has_end_v<const std::vector<int>&>, "const vector ref");
...
TEST(AddressToStringTest, TupleToString) {  // NOLINT
  ASSERT_EQ(to_string(std::tuple<>{}), "");
  ASSERT_EQ(to_string(std::tuple<int>{42}), "42");
  ASSERT_EQ(to_string(std::tuple<int>{24}), "24");
  ASSERT_EQ(to_string(std::tuple<int, int>{42, 24}), "42.24");
  ASSERT_EQ(to_string(std::tuple<int, int, int>{42, 24, 42}), "42.24.42");
  ASSERT_EQ(to_string(std::tuple<int, int, int, int>{42, 24, 42, 24}),
            "42.24.42.24");
}
